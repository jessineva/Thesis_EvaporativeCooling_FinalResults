...
class EvapMistCooling(EnergyPlusPlugin):
    ...
    def get_result_temperature(self, state, Tbs, RH, pa, Qin, eff_ev, nozzles):
        """
        ...
        if RH == 100:
            Tee = Tbs
            hum = psychro.humidity_ratio_c(state, Tbs, 1, pa)
            Tbh = Tbs
            return Tee
        else:    
            SpFlow  = 1.135e-8*5 #m3/s
            w = psychro.humidity_ratio_c(state, Tbs, RH/100, pa)
            Tbh = psychro.wet_bulb(state, Tbs, w, pa)
            m_a = Qin / psychro.specific_volume(state, Tbs, w, pa)
            m_w = w * m_a
            ...
            if Tee<=Tbh:
                Tee = Tbh
                hum = psychro.humidity_ratio_c(state, Tbh, 1, pa)
                RH  = 100
                
            return Tee
    
    def get_result_hum(self, state, Tbs, RH, pa, Qin, eff_ev, nozzles):
        """
        Calculates the result temperature based on 100% evaporation of water in the fictional evaporative zone.
    ...
    def get_result_tbh(self, state, Tbs, RH, pa, Qin, eff_ev, nozzles):
        """
        Calculates the result temperature based on 100% evaporation of water in the fictional evaporative zone.
    ...
    def get_rel_hum(self, state, Tbs, hum, Pb):
        """
        ...
        """
        psychro = self.api.functional.psychrometrics(state)
        RH      = psychro.relative_humidity_b(state, Tbs, hum, Pb)*100
        if RH>100:
            RH = 100
        return RH
    ...
    def get_handles(self, state):
        ...
        self.senTbs = self.api.exchange.get_variable_handle(state, u"ZONE MEAN AIR TEMPERATURE", keyCAF)
        self.senTbh = self.api.exchange.get_variable_handle(state, u"SITE OUTDOOR AIR WETBULB TEMPERATURE", keyENV)
        self.senRH  = self.api.exchange.get_variable_handle(state, u"ZONE AIR RELATIVE HUMIDITY", keyCAF)
...
#         self.senPBCafe1_Q12 = self.api.exchange.get_variable_handle(state, mtnQ12, u"NCAFETERIA_ENTRADA_WE")
#         self.senPBCafe1_Q21 = self.api.exchange.get_variable_handle(state, mtnQ21, u"NCAFETERIA_ENTRADA_WE")
#         self.senPBCafe2_Q12 = self.api.exchange.get_variable_handle(state, mtnQ12, u"NCAFETERIA_VENTANAOESTE")
#         self.senPBCafe2_Q21 = self.api.exchange.get_variable_handle(state, mtnQ21, u"NCAFETERIA_VENTANAOESTE")
        ...
        self.PBCafe1_tbh   = self.api.exchange.get_global_handle(state, u"PBCafe1_tbh")
        self.PBCafe1_hum = self.api.exchange.get_global_handle(state, u"PBCafe1_hum")
...
#         Tbh = self.api.exchange.get_variable_value(state, self.senTbh)
        RH  = self.api.exchange.get_variable_value(state, self.senRH)
...
#                     - self.api.exchange.get_variable_value(state, self.senPBCafe2_Q21)
        ...
        PBCafe1_Qin = (9.25*9.83*3.5) + (2.7*14*3.5)
...
#         self.api.exchange.set_global_value(state, self.gvPBCafe2_Qin, PBCafe2_Qin)
        ...
        self.api.exchange.set_global_value(state, self.PBCafe1_Tee, self.get_result_temperature(state, Tbs, RH, Pb, PBCafe1_Qin, effev1, 14))
        self.api.exchange.set_global_value(state, self.PBCafe1_hum, self.get_result_hum(state, Tbs, RH, Pb, PBCafe1_Qin, effev1, 14))
        self.api.exchange.set_global_value(state, self.PBCafe1_tbh, self.get_result_tbh(state, Tbs, RH, Pb, PBCafe1_Qin, effev1, 14))
        
        self.api.exchange.set_global_value(state, self.PBCafe1_eff, self.get_effectiveness(Tbs, self.api.exchange.get_global_value(state, self.PBCafe1_tbh), self.api.exchange.get_global_value(state, self.PBCafe1_Tee)))
        self.api.exchange.set_global_value(state, self.PBCafe1_RHee, self.get_rel_hum(state, self.api.exchange.get_global_value(state, self.PBCafe1_Tee), self.api.exchange.get_global_value(state, self.PBCafe1_hum), Pb))
...
#         self.api.exchange.set_global_value(state, self.PBCafe2_RHee, self.get_rel_hum(state, self.api.exchange.get_global_value(state, self.PBCafe1_Tee), Tbh, Pb))   AQUI HAY UN ERROR
